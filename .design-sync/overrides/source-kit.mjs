// forked from design-sync lib/source-kit.mjs — honor componentSrcMap nulls when synthesizing the entry
//
// Why this fork exists (Trixode site, synth-entry mode — the repo is a Next.js app
// with no library build, so the entry is synthesized from components/ui/*.tsx):
// upstream builds that synthesized entry from ALL discovered src files, before
// componentSrcMap exclusions are applied. So `componentSrcMap: {"Calendar": null}`
// dropped the component from the card list but still `export * from`'d
// components/ui/calendar.tsx — which imports react-day-picker, a package this repo
// does not install (calendar.tsx is dead code here). That hard-fails the esbuild
// bundle with [UNRESOLVED_IMPORT].
//
// The change is one filter at the synth-entry step, and it is general: a component
// mapped to null is excluded from the synthesized entry too, which is what the null
// already reads as. No other behaviour differs from upstream.
//
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    // FORK: exclude files belonging to null-mapped components. Uses the same
    // <Name>.tsx | <name>/<name>.tsx | <Name>/index.tsx | <kebab-name>.tsx
    // matching as the step-4 fuzzy-find below, so the two agree on what a
    // component's source file is.
    const nameRxFor = (n) => {
      const kebab = n.replace(/([a-z0-9])([A-Z])/g, '$1-$2');
      return `${n}/(?:index|${n})\\.(?:tsx|jsx)|(?:${n}|${kebab})\\.(?:tsx|jsx)`;
    };
    const nulled = Object.entries(srcMap).filter(([, v]) => v === null).map(([k]) => k);
    const excludeRx = nulled.length
      ? new RegExp(`(?:^|/)(?:${nulled.map(nameRxFor).join('|')})$`, 'i')
      : null;

    // FORK: a pinned src path is the authority for that name — drop OTHER files
    // that would also claim it. `export * from` both of two files exporting the
    // same symbol makes that export ambiguous, and ES star-export semantics drop
    // ambiguous names entirely, so the symbol lands `undefined` on the global
    // ([BUNDLE_EXPORT]). Here: both sonner.tsx and toaster.tsx export `Toaster`.
    const pinned = Object.entries(srcMap).filter(([, v]) => typeof v === 'string');
    const shadowed = [];
    for (const [name, rel] of pinned) {
      const keep = slash(resolve(PKG_DIR, rel));
      const rx = new RegExp(`(?:^|/)(?:${nameRxFor(name)})$`, 'i');
      for (const p of srcFiles) {
        if (rx.test(p) && slash(p) !== keep) shadowed.push(slash(p));
      }
    }
    ...
    const comps = srcFiles.filter(
      (p) =>
        SRC_IMPL_RX.test(p) &&
        !NON_IMPL_RX.test(p) &&
        !(excludeRx && excludeRx.test(p)) &&
        !shadowed.includes(slash(p)),
    );
    if (excludeRx) {
      console.error(`[OVERRIDE] source-kit: excluded ${nulled.join(', ')} from the synthesized entry`);
    }
    if (shadowed.length) {
      console.error(
        `[OVERRIDE] source-kit: ${shadowed.map((p) => p.split('/').pop()).join(', ')} shadowed by a componentSrcMap pin`,
      );
    }
  ...
  // FORK: in synth-entry mode the src scan IS the authority for the component
  // list (there is no .d.ts tree, so `exportedNames` returns empty). Upstream
  // only falls back to the src scan when the name set is completely empty, so a
  // single non-null componentSrcMap pin silently suppressed discovery of all
  // other components — 246 collapsed to 1. A pin must ADD to discovery, not
  // replace it, so union the two here.
  if (synthEntry) {
    const have = new Set(components.map((c) => c.name));
    for (const c of deriveComponentsFromSrc(srcFiles)) {
      if (srcMap[c.name] === null || have.has(c.name)) continue;
      components.push(c);
      have.add(c.name);
    }
